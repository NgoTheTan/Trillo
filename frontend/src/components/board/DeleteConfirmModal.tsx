...
export const DeleteConfirmModal: React.FC<DeleteConfirmModalProps> = ({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[420px] p-6 rounded-2xl bg-white border border-slate-100 shadow-xl">
        <DialogHeader className="flex flex-col items-center text-center gap-2 pt-2">
          ...
          <DialogDescription className="text-sm text-slate-500 max-w-xs">
            {description || (
              <>
                Are you sure you want to delete <span className="font-semibold text-slate-800">"{boardTitle}"</span>? This action cannot be undone and all cards inside will be permanently removed.
              </>
            )}
          </DialogDescription>
        </DialogHeader>
